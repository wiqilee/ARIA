...
- A patient-specific risk score on a 0 to 10 scale, with clinical interpretation. The numeric score is mapped to a severity label using a deterministic, single-source-of-truth threshold (`0.0–2.0` LOW, `2.0–5.0` MODERATE, `5.0–8.5` HIGH, `8.5–10.0` CRITICAL) that is identical across the Rust MCP server, the Python agent, and the Vercel frontend, so a score of 9.4 is always CRITICAL no matter where it is rendered.
...
| Drug interaction alerts overridden by clinicians | 90% pooled (CI 85–95%) | [Felisberto et al., 2024](https://pubmed.ncbi.nlm.nih.gov/38899788/) |
...
│   │   │   ├── score_risk.rs           # Patient phenotype risk multiplier + deterministic severity_label_for_score()
...
│   │       ├── severity.ts             # Deterministic score→label & color mapping (single source of truth)
...
ARIA's `0–10` risk score is mapped to a categorical label using a deterministic threshold function that is the single source of truth across every component of the system:
...
- [x] Deterministic severity threshold (`severity_label_for_score`) mirrored across Rust MCP server, Python agent, and Vercel frontend so score-to-label mapping is identical on every surface (Vercel, A2A, Prompt Opinion)
